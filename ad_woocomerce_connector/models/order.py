...
class WooOrder(models.Model):
    ...
    def push_order_to_store(self):
        from ..components.exporter import WooFullOrderExporter
        exporter = WooFullOrderExporter()

        def _op(binding):
            binding._ensure_line_products_mapped()
            exporter.run(binding.backend_id, binding)
            binding.mark_synced("Order pushed to WooCommerce.")

        return self._run_with_notification(_op, title="Push Order Complete")

    def _ensure_line_products_mapped(self):
        """Auto-push any line product missing a mapping on this backend."""
        self.ensure_one()
        ...
        for line in self.order_id.order_line:
            ...
            if not binding.external_id:
                binding.with_context(return_raw_results=True).push_to_store()
    ...
    def debug_dump_order_json(self):
        """Fetch and log the full WooCommerce order JSON + tracking endpoint,
        so we can see where shipping/tracking data lives. Temporary diagnostic."""
        import json
        for binding in self.filtered("external_id"):
            ...
            data = result.get("data", {})
            _logger.info(
                "[WC ORDER JSON] order %s:\n%s",
                binding.external_id, json.dumps(data, indent=2, default=str),
            )
            # Probe common shipment-tracking plugin endpoints.
            for ep in (
                "orders/%s/shipment-trackings" % binding.external_id,
                "orders/%s/shipment_trackings" % binding.external_id,
            ):
                try:
                    tr = client.get(ep)
                    _logger.info("[WC TRACKING] %s ->\n%s", ep,
                                 json.dumps(tr.get("data"), indent=2, default=str))
                except Exception as exc:
                    _logger.info("[WC TRACKING] %s -> not available (%s)", ep, exc)
        return {
            "type": "ir.actions.client",
            "tag": "display_notification",
            "params": {
                "title": "Debug",
                "message": "Order JSON dumped to log. Check for shipping/tracking fields.",
                "type": "success",
                "sticky": False,
            },
        }
...
class SaleOrderWoo(models.Model):
    ...
    wc_is_pushed = fields.Boolean(
        string="Pushed to WooCommerce",
        compute="_compute_wc_is_pushed",
        help="Set when this order already exists in at least one WooCommerce store.",
    )

    @api.depends("wc_bind_ids", "wc_bind_ids.external_id")
    def _compute_wc_is_pushed(self):
        for order in self:
            order.wc_is_pushed = any(
                b.external_id for b in order.wc_bind_ids
            )
    ...
    def action_push_wc_order(self):
        """Open a wizard to pick the WooCommerce store, then push."""
        if not self.env["wc.store"].search_count([("active", "=", True)]):
            raise UserError("No active WooCommerce stores found.")
        return {
            "name": "Push Order to WooCommerce",
            "type": "ir.actions.act_window",
            "res_model": "wc.order.push.wizard",
            "view_mode": "form",
            "target": "new",
            "context": {"default_order_ids": self.ids},
        }

    def _wc_is_paid(self):
        """True when the order has posted customer invoices and all are fully paid."""
        self.ensure_one()
    ...
    def _push_to_backend(self, backend):
        """Create bindings (if missing) for these orders on `backend` and push.

        Only fully-paid orders are pushed to WooCommerce."""
        unpaid = self.filtered(lambda o: not o._wc_is_paid())
        if unpaid:
            raise UserError(
                "These orders are not fully paid and cannot be pushed to "
                "WooCommerce:\n%s" % "\n".join(unpaid.mapped("name"))
            )
        bindings = self.env["wc.order"]
        for order in self:
            binding = order.wc_bind_ids.filtered(
            ...
            bindings |= binding
        return bindings.push_order_to_store()
